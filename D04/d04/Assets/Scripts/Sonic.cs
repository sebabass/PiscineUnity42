...
public class Sonic : MonoBehaviour
{
	...
	public AudioSource aSign;

	public static Sonic sn;
	...
	void Update() {
		if (isSign) {
			gameManager.gm.sign = true;
			timeSign -= Time.deltaTime;
			if (timeSign <= 0 && !isScore) {
				isScore = true;
				gameManager.gm.addMenuScore ();
			}
			sign.Rotate (new Vector3 (0.0f, 1000.0f * Time.deltaTime, 0.0f));
		} else 
	...
	void OnTriggerEnter2D(Collider2D collision) {
		...
		if (collision.gameObject.tag == "ring" && !isSign) {
			GameObject.Destroy (collision.gameObject);
			aRing.Play();
			rings++;
			user.us.UpdateNbRings();
		}
		if (collision.gameObject.tag == "sign" && !isSign) {
			isSign = true;
			aMusic.loop = false;
			aMusic.Stop();
			aSign.Play();
		...
		if (collision.gameObject.tag == "pic" && !isSign) {
			getHit();
		}
	}
	...
	void randRingsEject () {
		int ringsSave = Mathf.RoundToInt(rings / 2);
		float randX = 0;
		float randY = 0;
		for (int i = 0; i<ringsSave; i++) {
			randX = Random.Range(1.0f, 10.0f);
			randY = Random.Range(1.0f, 10.0f);
			GameObject tmp = (GameObject)Instantiate(gmrings, new Vector3(transform.position.x - randX, transform.position.y - randY, 0), Quaternion.identity);
			StartCoroutine(ringsFlash(tmp));
		}
	}
	...
	IEnumerator ringsFlash (GameObject r) {
		...
		//StartCoroutine (ringsDestroy(r));
	}

	IEnumerator ringsDestroy (GameObject r) {
		if (r != null) {
